...
const AddPost = () => {
  ...
  const submitHandler = async () => {
    ...
    try {
      ...
      const { data } = await axios.post(`https://liaison-main-4oyd.onrender.com/api/posts/addPost`, { post: pic, caption }, config);
      toast({
        title: "Post added Successfully",
        status: 'success',
        duration: 5000,
  ...
  return (
    ...
    <div className='addpost min-h-screen w-5/6 bg-black flex items-center justify-center'>
      <div className='addpost2 w-[50%] max-w-[500px] mx-auto p-4 text-slate-300  border rounded shadow-lg bg-zinc-900 '>
        <h2 className="text-2xl font-bold mb-4">Create New Post</h2>
        ...
        <textarea
          ...
          value={caption}
          onChange={(e) => setCaption(e.target.value)}
          className="w-full px-3 py-2 mb-4 border bg-zinc-800 rounded resize-none"
          rows="4"
        />
